...
public class GrabbingHand : MonoBehaviour {
  ...
  void Start() {
    ...
    palm_rotation_ = Quaternion.identity;
        contHover = GameObject.Find("ControlTest").GetComponent<TestLeapLeap>();
  }
  ...
  protected Collider FindClosestGrabbableObject(Vector3 pinch_position) {
    ...
    // Debug.Log("Are you even?");
    // HandModel hand_model = GetComponent<HandModel>();
    // Vector3 pinchp = 0.5f * (hand_model.fingers[0].GetTipPosition() + 
      ...
      if (close_things[j].GetComponent<Rigidbody>() != null && sqr_distance < closest_sqr_distance &&
        ...
        if (grabbable == null || !grabbable.IsGrabbed()) {  
          ...
          closest_sqr_distance = sqr_distance;
            ...
            if (close_things[j].gameObject.tag == "Flow" && sqr_distance < 4)
            {
                //Debug.Log(closest_sqr_distance);
                contHover.hover(close_things[j].gameObject);
            }
    }
  ...
  public Vector3 GetIndexPos() {
      HandModel hand_model = GetComponent<HandModel>();
      Hand leap_hand = hand_model.GetLeapHand();

      Vector leap_index_tip = leap_hand.Fingers[0].TipPosition;

      Vector3 unityPosition = leap_index_tip.ToUnityScaled(false);
      Vector3 worldPosition = transform.TransformPoint(unityPosition);
